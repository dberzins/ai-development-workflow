...
This document is the human narrative source. The AI-facing rules layer is separate:

- `RULES_REGISTRY.md` is the canonical AI-rules source. It holds all rule text with stable IDs and a prompt→rule mapping table.
- Each task prompt in `.github/prompts/` carries only the rules relevant to it, inlined inside `<!-- RULES:BEGIN -->` / `<!-- RULES:END -->` delimiters generated by `/re-sync-rules`.
- `COMPACT_WORKFLOW.md` is the human quick reference. It includes the Document Registry table and is the right file to attach when providing manual workflow context to an AI assistant.
- `workflow.prompt.md` has been removed. Do not recreate it.

To add or change a rule: edit `RULES_REGISTRY.md`, then run `/re-sync-rules` for the affected prompt files. Material changes to this document require a `RULES_REGISTRY.md` review in the same pull request., then run the `Export Mermaid diagrams` VS Code task or execute `scripts/export-mermaid-diagrams.ps1` from the repository root.
...
1. **Minimal package**
  - `RULES_REGISTRY.md` — canonical AI rules source
  - `templates/` — canonical scaffold files for document generation
  - `.github/prompts/` — reusable task prompt files
2. **Enhanced package**
...
1. Do not implement large features without a related analysis or plan. `(CR6)`
2. Do not make major architectural choices without an ADR or recorded decision. `(CR3, CR7)`
3. Do not mark a feature complete without a validation record and a quality review pass. `(CR8)`
4. Do not close issues without recording root cause and fix summary.
5. Do not publish without `PUBLISH.md` review.
6. Do not treat `README.md` as a substitute for technical design docs. `(CC4)`
7. Do not leave checklists, risk entries, or traceability links stale after a session. `(G9)`
8. Do not create duplicate feature names across artifacts.

## Rule Traceability

`RULES_REGISTRY.md` is the canonical source for all rule text and IDs. The table below
cross-references where each rule group originates in this document so contributors can
locate the narrative behind a rule without searching the full file.

| Rule Group | Rule IDs | Originates in WORKFLOW.md section |
|---|---|---|
| Core Rules | CR1–CR8 | [Core Principles](#core-principles) |
| Workflow Sequence | WS1 | [Final Recommendation](#final-recommendation), [Workflow Stages](#workflow-stages) table |
| Document Guardrails | G1–G9 | [Document Status Metadata](#document-status-metadata), [Required Documents](#required-documents) |
| Checklist Rules | C1–C4 | [Checklist and Status Tracking](#checklist-and-status-tracking) |
| Session Guidance | S1–S3 | [Session Management](#session-management) |
| Content Constraints | CC1–CC7 | per-document Rules subsections, [Document Tiers](#document-tiers) |

To add or change a rule: edit `RULES_REGISTRY.md`, then run `/re-sync-rules` for the
affected prompt files. Do not edit `RULES:BEGIN/END` blocks in prompt files manually—
...
## Quick-Start Command Mapping

Map each workflow step to its corresponding prompt file for easy discovery in Copilot Chat:

| Workflow Step | Prompt CMD | Template | Purpose |
|---|---|---|---|
| Project bootstrap | `/bootstrap-workflow` | ARCHITECTURE.md, setup.md, ROADMAP.md, RISKS.md | Create initial documentation set |
| Feature discovery | `/feature-analysis` | FEATURE_ANALYSIS.md | Compare options and recommend approach |
| Planning | `/feature-plan` | FEATURE_PLAN.md | Create phased execution plan |
| Implementation | `/implementation` | FEATURE_PLAN.md (execution) | Execute phase by phase |
| Validation planning | `/test-plan` | TEST_PLAN.md | Define validation strategy |
| Quality review | `/quality-review` | Result validation | Review for bugs and risks |
| Issue capture | `/issue-capture` | FEATURE_ISSUE.md | Document debugging findings |
| Decision record | `/decision-record` | ADR.md | Record architecture decisions |
| Refactor review | `/refactor-review` | FEATURE_REFACTOR.md | Identify cleanup opportunities |
| Publish readiness | `/publish-readiness` | PUBLISH.md | Verify release readiness |
| Session start | `/session-start` | Session context | Load context and confirm status |
| Session end | `/session-end` | Session summary | Update docs and record next steps |
| Retrospective | `/retrospective` | RETROSPECTIVE.md | Capture post-milestone lessons |
